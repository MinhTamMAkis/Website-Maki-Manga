                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{route('chapter.update',[$chapter->id])}}" enctype='multipart/form-data'>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" value="{{$chapter->tieude}}" name="tieude" onkeyup="ChangeToSlug();" id="slug" aria-describedby="Tên chapter ....">
                        </div>

                        <div class="mb-3">
                            <label for="convert-slug" class="form-label">Slug chapter</label>
                            <input type="text" class="form-control" value="{{$chapter->slug_chapter}}" name="slug_chapter" id="convert_slug" aria-describedby="Slug chapter ....">
                        </div>
                        <!-- tóm tắt chapter -->
                        <div class="mb-3">
                            <label for="convert-slug" class="form-label">Tóm tắt chapter</label>
                            <input type="text" class="form-control" value="{{$chapter->tomtat}}" name="tomtat" id="convert_slug" aria-describedby="">
                        </div>

                        <div class="mb-3">
                            <label for="motadanhmuc" class="form-label">nội dung</label>
                            <textarea class="form-control" name="noidung" value="{{$chapter->noidung}}" rows="5" style="resize: none;" id="tomtat"></textarea>
                        </div>

                        <div class="mb-3">
                            <label for="convert-slug" class="form-label">Hình ảnh</label><br>
                            <input type="file" class="custom-file-input"  name="hinhanh[]" multiple>
                            @foreach(json_decode($chapter->hinhanh, true) as $image) 
                                <img src="{{URL::to($image)}}" alt="" width="100" height="100">
                            @endforeach
                        </div>
                        <div class="mb-3">
                            <label for="kichhot" class="form-label">Thuộc truyện</label>
                            <select name="truyen_id" class="form-select" >
                                @foreach($truyen_id as $key => $value)
